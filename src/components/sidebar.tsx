...
// //     border-r border-r-gray-100 dark:border-r-gray-600
...
// //   ${
// //     pathname === item.path
// //       ? `my-gradient  text-blue-700 border-l-4 border-[#2196f3] dark:bg-gray-700 dark:text-white`
...
export default function Sidebar({ collapsed }: SidebarProps) {
  ...
  const handleLinkClick = (path?: string, label?: string) => {
    if (path) {
      setOpenSubMenu(null); // ✅ close popup immediately
      router.push(path);
    } else if (label) {
      setOpenSubMenu(openSubMenu === label ? null : label);
    }
    ...
    >
      <ul className="mt-4 px-2 relative">
        {menuItems.map((item) => {
          ...
          const isOpen = openSubMenu === item.label; // only controlled by click
