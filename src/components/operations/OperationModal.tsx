...
const OperationModal: React.FC<OperationModalProps> = ({ isOpen, onClose, onSubmit, editingItem ,isReadOnly}) => {
    ...
    // Effect to pre-populate form when editingItem changes
    useEffect(() => {
        if (editingItem) {
            setFormData({
            ...
            >
                ...
                <div className="flex justify-between items-center p-5 border-b border-gray-100 bg-gray-50 rounded-t-xl">
                    <h3 className="text-xl font-semibold text-gray-800">
                        {isReadOnly?'View Operation Details' :isEditing ? 'Edit Operation Details' : 'Add New Operation'}
                    </h3>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="operation" value={formData.operation} onChange={handleChange} required className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 border text-xs" />
                        </div>
                        
                        {/* SMV */}
                        <div>
                            <label className="block text-xs font-medium text-gray-700">SMV</label>
                            <input type="number" step="0.01" name="smv" value={formData.smv} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 border text-xs" />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="machineCode" value={formData.machineCode} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 border text-xs" />
                        </div>
                        {/* Master Operation */}
                        <div>
                            <label className="block text-xs font-medium text-gray-700">Master Operation</label>
                            <input type="text" name="masterOperation" value={formData.masterOperation} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 border text-xs" />
                        </div>
                        {/* Skill Grade */}
                        <div>
                            <label className="block text-xs font-medium text-gray-700">Skill Grade</label>
                            <input type="text" name="skillGrade" value={formData.skillGrade} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 border text-xs" />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="hindi" value={formData.hindi} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 border text-xs" />
                        </div>
                        {/* Tamil */}
                        <div>
                            <label className="block text-xs font-medium text-gray-700">Tamil Translation</label>
                            <input type="text" name="tamil" value={formData.tamil} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 border text-xs" />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="comments" value={formData.comments} onChange={handleChange} rows={2} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 border text-xs resize-none"></textarea>
                    </div>
                    
                    {/* Action Buttons (Footer) */}
                    <div className="flex justify-end space-x-3 pt-4 border-t border-gray-100">
                        <button type="button" onClick={onClose} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 text-xs font-medium transition">
                            Cancel
                        </button>
                        {!isReadOnly && (
                        <button type="submit" className="px-5 py-2 bg-blue-600 text-white rounded-lg shadow-md hover:bg-blue-700 text-xs font-medium transition">
